// Stolen from https://github.com/JetBrains/kotlin/blob/master/libraries/stdlib/test/collections/CollectionTest.kt

import kotlin.math.pow
import kotlin.random.Random
import kotlin.test.*

private inline fun <T> List(size: Int, init: (index: Int) -> T): XorLinkedList<T> {
    val list = XorLinkedList<T>()
    repeat(size) { index -> list.add(init(index)) }
    return list
}

private inline fun <T> MutableList(size: Int, init: (index: Int) -> T): XorLinkedList<T> = List(size, init)

private fun <T> listOf(vararg elements: T) = XorLinkedList<T>().apply { addAll(elements) }
private fun <T> arrayListOf(vararg elements: T) = XorLinkedList<T>().apply { addAll(elements) }
private fun <T> emptyList() = XorLinkedList<T>()

class CollectionTest {

    @Test
    fun createListWithInit() {
...
//        assertEquals(src.toList(), anyValues)
    ...
    @Test
    fun foldIndexedWithDifferentTypes() {
        expect(10) {
    ...
    @Test
    fun foldIndexedWithNonCommutativeOperation() {
        expect(4) {
    ...
    @Test
    fun foldRightIndexedWithDifferentTypes() {
        expect("12343210") {
            val numbers = listOf(1, 2, 3, 4)
    ...
    @Test
    fun foldRightIndexedWithNonCommutativeOperation() {
        expect(-4) {
            val numbers = listOf(1, 2, 3)
    ...
    @Test
    fun foldWithNonCommutativeOperation() {
        expect(1) {
            val numbers = listOf(1, 2, 3)
    ...
    @Test
    fun foldRightWithDifferentTypes() {
        expect("1234") {
            val numbers = listOf(1, 2, 3, 4)
    ...
    @Test
    fun foldRightWithNonCommutativeOperation() {
        expect(-5) {
            val numbers = listOf(1, 2, 3)
    ...
    fun scanIndexed() {
        for (size in 0 until 4) {
            ...
            assertEquals(
                expected,
                List(size) { 'a' + it }.runningFoldIndexed("+") { index, acc, e -> "$acc[$index: $e]" })
        }
    }
    ...
    fun associateWith() {
        ...
        val updatedLength =
            items.drop(1).associateWithTo(itemsWithTheirLength.toMutableMap()) { name ->
                name.lowercase().count { it in "aeuio" }
            }
    ...
    @Test
    fun plusElement() = testPlus { it + "cheese" + "wine" }
    @Test
    fun plusCollection() = testPlus { it + listOf("cheese", "wine") }
    @Test
    fun plusArray() = testPlus { it + arrayOf("cheese", "wine") }
    @Test
    fun plusSequence() = testPlus { it + sequenceOf("cheese", "wine") }

    @Test
    fun plusCollectionBug() {
        val list = listOf("foo", "bar") + listOf("cheese", "wine")
        assertEquals(listOf("foo", "bar", "cheese", "wine"), list)
    }

    @Test
    fun plusCollectionInference() {
    ...
    @Test
    fun minusElement() = testMinus(expected = listOf("foo", "bar")) { it - "bar" - "zoo" }
    @Test
    fun minusCollection() = testMinus { it - listOf("bar", "zoo") }
    @Test
    fun minusArray() = testMinus { it - arrayOf("bar", "zoo") }
    @Test
    fun minusSequence() = testMinus { it - sequenceOf("bar", "zoo") }
    ...
    fun sorted() {
        ...
        val dataDouble = listOf(
            Double.POSITIVE_INFINITY, Double.NEGATIVE_INFINITY, Double.MIN_VALUE, -Double.MIN_VALUE,
            1.0, -1.0, Double.MAX_VALUE, -Double.MAX_VALUE, Double.NaN, 0.0, -0.0
        )
        assertEquals(
            listOf(
                Double.NEGATIVE_INFINITY, -Double.MAX_VALUE, -1.0, -Double.MIN_VALUE, -0.0,
                0.0, Double.MIN_VALUE, 1.0, Double.MAX_VALUE, Double.POSITIVE_INFINITY, Double.NaN
            ), dataDouble.sorted()
        )
        assertEquals(
            listOf(
                Double.NaN, Double.POSITIVE_INFINITY, Double.MAX_VALUE, 1.0, Double.MIN_VALUE, 0.0,
                -0.0, -Double.MIN_VALUE, -1.0, -Double.MAX_VALUE, Double.NEGATIVE_INFINITY
            ), dataDouble.sortedDescending()
        )
    }
    ...
    fun toStringTest() {
        ...
        assertEquals("[1, a, null, ${Long.MAX_VALUE}]", listOf(1, "a", null, Long.MAX_VALUE).toString())
    }

    @Test
    fun toStringContainingThis() {
