// Stolen from https://github.com/JetBrains/kotlin/blob/master/libraries/stdlib/test/collections/IterableTests.kt

import kotlin.test.*

class XorLinkedListTest : OrderedIterableTests<Iterable<String>>(
    createFrom = { XorLinkedList<String>().apply { addAll(it) } },
    XorLinkedList()
)

abstract class OrderedIterableTests<T : Iterable<String>>(createFrom: (Array<out String>) -> T, empty: T) :
    IterableTests<T>(createFrom, empty) {
    @Test
    fun indexOf() {
...
fun <T> Iterator<T>.assertSorted(isInOrder: (T, T) -> Boolean) {
    ...
    while (hasNext()) {
        ...
        assertTrue(
            isInOrder(prev, next),
            "Not in order at position $index, element[${index - 1}]: $prev, element[$index]: $next"
        )
        prev = next
    }
    return
